...
figure1 <- multi_panel_figure(
  ...
  unit = "cm",
  inter_row_spacing = c(0, rep(0.5, 6)),
  inter_column_spacing = c(0, rep(0.5, 2)))
...
```{r noeval_template_generation_call, code=formatR::usage('scramblase_assay_input_template'), eval=FALSE}
```
...
```{r noeval_traces_call, code = formatR::usage('scramblase_assay_traces'), eval = FALSE}
```
...
```{r noeval_plot_call, code = formatR::usage('scramblase_assay_plot'), eval = FALSE}
```
...
```{r noeval_stats_call, code = formatR::usage('scramblase_assay_stats'), eval = FALSE}
```
...
scramblase_assay_traces("inputTable.txt", time_max_sec = 350)
```

```{r figure1_creation, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "***flippant*--based reanalysis of data in Figure 4C/D from @ploier_perturbed_submitted.** (A) represents spectral traces as produced by a call to the function `scramblase_assay_traces`, while (B) and (C) show protein to phospholipid ration (PPR) plots for the data using a x--axis trimmed to a maximum value of 2 (see text for details). In (B) data is separately plotted for independent experiments, while (C) combines all data points, aiming for reliability of fit. 'adj. PPR' indicates that the measure has been scaled to account for a vesicle pool refractory to flippase reconsitution."}
x <- flippant:::read_scramblase_input_file(
...
figure1 <- add_panel(
  ...
  left_panel = 1, 
  right_panel = 3, 
  panel = scramblase_assay_traces(x, time_max_sec = 350) + theme_bw())
figure1 <- add_panel(
  figure = figure1,
  top_panel = 4,
  bottom_panel = 5,
  left_panel = 1, 
  right_panel = 3, 
  panel = scramblase_assay_plot(x, generation_of_algorithm = 2) + 
    theme_bw() + 
    xlim(0, 2) + 
...
figure1 <- add_panel(
  ...
  left_panel = 1,
  right_panel = 3,
  panel = scramblase_assay_plot(x, generation_of_algorithm = 2, split_by_experiment = FALSE) + 
    theme_bw() + 
    xlim(0, 2) + 
    xlab(expression("adj. PPR"~~("mg"/"mmol"))) +
    ylab(expression(P[{phantom()>=1}])) +
...
scramblase_assay_plot("inputTable.txt", generation_of_algorithm = 2) + xlim(0, 2)
```
...
scramblase_assay_stats("inputTable.txt", generation_of_algorithm = 2, split_by_experiment = FALSE)
```
...
ppr_results <- scramblase_assay_stats(
  ...
  mutate(`Fit Constant` = round(`Fit Constant` * 10E3, 2)) %>%
  set_names(c("Experimental Series", "Fit Constant ($\\times 10^3$)"))
...
ppr_results_alpha_print <- ppr_results %>%
  extract2("Fit Constant ($\\times 10^3$)") %>%
  round(2) %>%
  head(n = nrow(ppr_results) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(ppr_results$`Fit Constant ($\\times 10^3$)`, n = 1))
knitr::kable(
  ppr_results,
  row.names = FALSE,
  caption = "**Tabular output of *flippant*--based reanalysis of data in Figure 4C/D from @ploier_perturbed_submitted.** An plain text--version of this output is produced by calling `scramblase_assay_stats`.")
```
...
\times 10^3 \approx 16$ was reported for mutant and $\alpha \times 10^3 \approx
