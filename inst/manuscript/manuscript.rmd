...
figure1 <- AddPanel(
  ...
  leftPanel = 1,
  rightPanel = 2,
  panel = scramblase_assay_plot(x, generation_of_algorithm = 1,  splitByExperiment = FALSE) + 
    theme_bw() + 
    xlim(0, 2) + 
    xlab(expression("PPR"~~("mg"/"mmol"))) +
    ylab(expression(P[{phantom()>=1}])))
...
scramblase_assay_stats("inputTable.txt", generation_of_algorithm = 1, splitByExperiment = FALSE)
```

```{r, echo = FALSE}
ppr_results <- scramblase_assay_stats(x, generation_of_algorithm = 1, splitByExperiment = FALSE)
