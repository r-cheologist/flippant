...
analysis_dir <- tempdir()
template_file <- "extdata/PloierEtAl_Data.zip" %>% 
  system.file(package = "flippant") %>% 
  unzip(files = "inputTable.txt", exdir = analysis_dir)
...
```{r define_template_file, echo = FALSE}
template_file <- "inputTable.txt"
```
...
x[["Path"]] <- file.path(analysis_dir, x[["Path"]])
