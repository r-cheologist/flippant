...
[see @menon_opsin_2011;@goren_constitutive_2014;@ploier_perturbed_submitted for
...
@ploier_perturbed_submitted and largely keeping with nomenclature used therein,
...
default and following @ploier_perturbed_submitted, PPR is scaled by a factor of
...
the maximum measurement in the series (`scale_to = "data"`) or the plateau of an
exponential fit to it (`scale_to = "model"`; for reasoning see above) and whether
or not to require the exponential curve fitted to the data to go through the
origin. `ppr_scale_factor` allows for the modification of scaling to account for
a pool of vesicles that is scramblase--inaccessible (see above). The
...
Figure 4C--F in @ploier_perturbed_submitted. The following work flow in a first 
step plots the corresponding spectral traces (Figure 1). Note that algorithmic
...
times between experiments. PPR scaling is used with the default factor of 0.65.
...
```{r figure1_creation, echo = FALSE, fig.height = 4, fig.cap = "**`flippant`--based plotting of spectral traces underlying the data in Figure 4C--F from @ploier_perturbed_submitted.** See text for command and details."}
# Plot spectral traces (Figure 1)
x <- tmpDir %>%
...
ppr_results <- scramblase_assay_stats(
    x, generation_of_algorithm = 2, split_by_experiment = FALSE) %>%
  mutate(`Fit Constant` = round(`Fit Constant` * 10^4, 2)) %>%
  set_names(c("Experimental Series", "Fit Constant ($\\times 10^4$)"))
ppr_results_names <- ppr_results %>%
  extract2("Experimental Series") %>%
  head(n = nrow(ppr_results) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(ppr_results$`Experimental Series`, n = 1))
ppr_results_alpha_print <- ppr_results %>%
  extract2("Fit Constant ($\\times 10^4$)") %>%
  round(2) %>%
  head(n = nrow(ppr_results) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
...
knitr::kable(
  ppr_results,
  row.names = FALSE,
  caption = "**Tabular output of `flippant`--based reanalysis of data in Figure 4C--F from @ploier_perturbed_submitted.** An plain text--version of this output is produced by calling `scramblase_assay_stats`.")
```
...
prior manual analysis by @ploier_perturbed_submitted (Table S2), where $\alpha
...
in @ploier_perturbed_submitted that the mutant rhodopsins investigated are
