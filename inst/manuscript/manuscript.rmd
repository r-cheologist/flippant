...
```
```{r, echo = FALSE, warning=FALSE, message=FALSE, fig.cap = "**`flippant`-based reanalysis of data in Figure 4C/D from @ploier_perturbed_submitted.** (A) represents spectral traces as produced by a call to the function `scramblaseAssayTraces`."}
setwd(tmpDir)
figure1 <- addpanel(
  figure = figure1,
  topPanel = 1, 
...
optimization and adaptation [@wickham_ggplot2_2009]. On a system running `Linux`
and `R` version `r paste(R.version[c("major","minor")], collapse = ".")`, the
statements generating equivalents to the plots and calculations presented here
take a mere `r analysis_timing_median` seconds (median; IQR: 
`r analysis_timing_iqr` s; $n=`r analysis_timing_n`$).
