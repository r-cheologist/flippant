...
knitr::opts_chunk$set(fig.width=6.78, fig.height=6.78, warning=FALSE, message=FALSE)
```
...
$F_{\text{min}}$ is analogously calculated from the last ten data points after 
a default 400 s or the time point of measurement supplied by the user in the 
parameter spreadsheet. $F_{\text{min}}$ is volume-corrected based on supplied 
reaction volumes with and without dithionite and for each spectrum/data point a 
