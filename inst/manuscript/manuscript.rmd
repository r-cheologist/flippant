...
experiments are integrated into a single analysis (`split_by_experiment = FALSE`)
or treated separately (`split_by_experiment = TRUE`; see below for example usage).
...
figure1 <- add_panel(
  ...
  left_panel = 1, 
  right_panel = 3, 
  panel = scramblase_assay_plot(x, generation_of_algorithm = 1) + 
    theme_bw() + 
    xlim(0, 2) + 
...
figure1 <- add_panel(
  ...
  left_panel = 1,
  right_panel = 2,
  panel = scramblase_assay_plot(x, generation_of_algorithm = 1,  split_by_experiment = FALSE) + 
    theme_bw() + 
    xlim(0, 2) + 
    xlab(expression("adj. PPR"~~("mg"/"mmol"))) +
    ylab(expression(P[{phantom()>=1}])))
...
scramblase_assay_plot("inputTable.txt", generation_of_algorithm = 1) + xlim(0, 2)
```
...
scramblase_assay_stats("inputTable.txt", generation_of_algorithm = 1, split_by_experiment = FALSE)
```

```{r analysis_tabular_output, echo = FALSE}
ppr_results <- scramblase_assay_stats(x, generation_of_algorithm = 1, split_by_experiment = FALSE)
