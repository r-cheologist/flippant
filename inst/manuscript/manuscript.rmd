...
figure1 <- AddPanel(
  ...
  leftPanel = 1, 
  rightPanel = 3, 
  panel = scramblaseAssayPlot(x, formulaGeneration = 1) + 
    theme_bw() + 
    xlim(0, 2) + 
...
figure1 <- AddPanel(
  ...
  leftPanel = 1,
  rightPanel = 2,
  panel = scramblaseAssayPlot(x, formulaGeneration = 1,  splitByExperiment = FALSE) + 
    theme_bw() + 
    xlim(0, 2) + 
    xlab(expression("PPR"~~("mg"/"mmol"))) +
    ylab(expression(P[{phantom()>=1}])))
...
scramblaseAssayPlot("inputTable.txt", formulaGeneration = 1) + xlim(0, 2)
```
...
scramblaseAssayStats("inputTable.txt", formulaGeneration = 1, splitByExperiment = FALSE)
```

```{r, echo = FALSE}
ppr_results <- scramblaseAssayStats(x, formulaGeneration = 1, splitByExperiment = FALSE)
