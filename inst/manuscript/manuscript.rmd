...
demonstrate its utility in a reanalysis of manual analysis from the publication
record, closely reproducing the reported results.
...
added graphical representations the PPR value equal to the fit 
...
$\alpha$, but are difficult to evaluate visually.
...
ppr_results_alpha_average <- ppr_results %>%
  split(ppr_results$`Experimental Series`) %>% 
  sapply(function(y){mean(y$`Fit Constant`, na.rm = TRUE)}) %>%
  round(2)
ppr_results_alpha_average_names <- names(ppr_results_alpha_average) %>%
  head(n = length(ppr_results_alpha_average) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(names(ppr_results_alpha_average), n = 1))
ppr_results_alpha_average_print <- ppr_results_alpha_average %>%
  unname() %>%
  head(n = length(ppr_results_alpha_average) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(ppr_results_alpha_average, n = 1))
knitr::kable(
  ppr_results,
  row.names = FALSE,
...
Mean values of fit constant $\alpha$) correspond with 
`r ppr_results_alpha_average_print` for `r ppr_results_alpha_average_names`, 
respectively, very well to the prior manual analysis by 
@ploier_perturbed_submitted, where $\alpha \approx 0.13$ was reported for mutant 
and $\alpha \approx 0.26$ for wild type rhodopsin. Based on the manual analysis 
of the data we conclude in @ploier_perturbed_submitted that the mutant 
