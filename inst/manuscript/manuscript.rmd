...
figure1 <- AddPanel(
  ...
  leftPanel = 1, 
  rightPanel = 3, 
  panel = scramblaseAssayPlot(x, generation_of_algorithm = 1) + 
    theme_bw() + 
    xlim(0, 2) + 
...
figure1 <- AddPanel(
  ...
  leftPanel = 1,
  rightPanel = 2,
  panel = scramblaseAssayPlot(x, generation_of_algorithm = 1,  splitByExperiment = FALSE) + 
    theme_bw() + 
    xlim(0, 2) + 
    xlab(expression("PPR"~~("mg"/"mmol"))) +
    ylab(expression(P[{phantom()>=1}])))
...
scramblaseAssayPlot("inputTable.txt", generation_of_algorithm = 1) + xlim(0, 2)
```
