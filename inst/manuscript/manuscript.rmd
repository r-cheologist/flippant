...
Numeric scramblase assay results such as $\tau$/$\alpha$ may be generated in 
tabular form using
...
step plots the corresponding spectral traces (Figure 1A). Note that alorithmic
...
figure1 <- addpanel(
  ...
  leftPanel = 1, 
  rightPanel = 3, 
  panel = scramblaseAssayTraces("inputTable.txt", timeMax = 350) + theme_bw())
figure1 <- addpanel(
  figure = figure1,
  topPanel = 4,
  bottomPanel = 5,
...
# Write out results in tabular manner
...
```
The first call results in the protein to phospholipid ratio (PPR) plot shown in
...
$\alpha$ or $\tau$, but are difficult to evaluate visually.

Recapitulation of the calculations underlying PPR plot generation by 
`scramblaseAssayPlot` in a call to `scramblaseAssayStats` produces tabular 
output represented in Table 1, which includes series-specific fit constants 
$\alpha$ in numeric and easily accessible form.
```{r, echo = FALSE}
setwd(tmpDir)
ppr_results <- scramblaseAssayStats("inputTable.txt")
ppr_results_tau_average <- ppr_results %>%
  split(ppr_results$`Experimental Series`) %>% 
  sapply(function(y){mean(y$`Fit Constant`, na.rm = TRUE)}) %>%
  round(2)
ppr_results_tau_average_names <- names(ppr_results_tau_average) %>%
  head(n = length(ppr_results_tau_average) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(names(ppr_results_tau_average), n = 1))
ppr_results_tau_average_print <- ppr_results_tau_average %>%
  unname() %>%
  head(n = length(ppr_results_tau_average) - 1) %>%
  paste(collapse = ", ") %>%
  paste("and") %>%
  paste(tail(ppr_results_tau_average, n = 1))
knitr::kable(
  ppr_results,
  row.names = FALSE,
  caption = "**Tabular output of `flippant`-based reanalysis of data in Figure 4C/D from @ploier_perturbed_submitted.** An `ASCII`-version of this output is produced by calling `scramblaseAssayStats`.")
```

Mean $\tau$ values (or fit constants $\alpha$) correspond with `r ppr_results_tau_average_print` for `r ppr_results_tau_average_names`, 
respectively, very well to the prior manual analyis by 
@ploier_perturbed_submitted, where $\tau \approx 0.12$ was reported for mustant 
and $\tau \approx 0.26$ for wild type rhodopsin. Based on the manual analysis of
the data we conclude in @ploier_perturbed_submitted that the mutant rhodopsins
investigated are reconsituted into proteoliposomes in monomeric form, contrary 
to the wild type, which is reconstituting as a homodimer. `flippant` analysis of
the data, shows identical trends and close matches of the results, supporting 
...
`R` [@r_core_team_r:_2015] as an environment for statistical computing and
graphics, scientists can quickly analyse such data and arrive at 
...
Results from `flippant`-driven analysis track well prior manual analysis. 
