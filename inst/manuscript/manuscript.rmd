...
To interrogate this functional class of proteins, we have developed a biochemical 
assay suited to characterize scramblase activity of purified proteins and protein 
extracts based on the reconstitution of scramblases or candidate proteins into 
preformed synthetic liposomes that contain trace amounts of nitrobenzoxadiazole 
(NBD) labeled fluorescent lipids [@chang_chemical_2004].
...
a default 400 s or the timepoint of measurement supplied by the user in the 
parameter spreadsheet. $F_{\text{min}}$ is volume-corrected based on supplied 
reaction volumes with and without dithionite and for each spectrum/datapoint a 
fluorescence reduction is calculated as follows:
$$
...
to the realtionship of $P_{\geq 1}$ to the protein to phospholipid ratio (PPR; 
...
In a first, graphical data analytics step of a scramblase assay-derived data 
set, fluorescent spectra or traces may be plotted in publication-appropriate
quality with acquisition time in the x- and fluorescense intensity on the y-axis
using
...
Plotting protein to phospholipid ratio (PPR) against the likelyhood for a given
...
function may be adjusted to use scramblase-mediated fluorescense reduction 
...
#Case Study
*Retinitis pigmentosa* is a degenerative disease of the retina, of which the 
majority of cases is linked to mutant forms of the G-protein coupled receptor 
...
Figure 5b in @ploier_perturbed_submitted.
