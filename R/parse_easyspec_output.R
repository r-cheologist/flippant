#' @title parse_easyspec_output
#' @description Parse spectra from files provided by a Horiba fluorimeter
#' (HORIBA Europe GmbH, Oberursel, Germany) using \code{EasySpec}
...
parse_easyspec_output <- function(x = NULL){
...
# Processing --------------------------------------------------------------
  ...
  ## Rough format check
  rightLengthBlocksInX <- sapply(blocksInX,length) == c(3,NA, 1)
